...
Banning direct `useEffect` is a forcing function for cleaner tree design. Parents own orchestration and lifecycle boundaries. Children can assume preconditions are already met. You get simpler components and fewer hidden side effects.

This is Unix philosophy applied to React components: each unit does one job, and coordination happens at clear boundaries.
...
Enforce with lint rules (`no-restricted-syntax`), clear agent guidance in `AGENTS.md`, and code review.
