...
In Codex, commands are plain text. If the user types a bare command like `kickoff`, `help`, `prep Stripe`, `mock panel`, or `salary`, treat that as an explicit command invocation. Natural-language requests that clearly map to one of the command workflows should be handled the same way.
...
Execute commands immediately when detected. If the user types the command name plainly or clearly asks for that workflow, treat it as an explicit command. Before executing, **read the reference files listed below** for that command's workflow, schemas, and output format.
