...
For Expo, install the SDK-compatible versions:

```bash
npx expo install react-native-reanimated react-native-worklets
```

Expo configures the Worklets Babel plugin through `babel-preset-expo`; no manual Babel entry is needed. Rebuild a development build after adding or upgrading the native dependencies. Expo Go already includes the SDK-supported Reanimated version.
...
Babel plugin: `'react-native-worklets/plugin'` (must be **last** in the plugins list), then rebuild the native app.
...
| Loop/pulse/shake | `withRepeat(anim, reps, reverse)` | v4: any non-positive `reps`; v3: use `-1` for infinite |
...
Requires `@react-navigation/native-stack`, Reanimated 4.2+, a native rebuild, and the static feature flag. It is unavailable in Expo Go. For setup and customization, read `references/shared-element-transitions.md`.
...
- v4: prefer `scheduleOnRN`/`scheduleOnUI` from `react-native-worklets`; `runOnJS`/`runOnUI` are deprecated compatibility APIs
