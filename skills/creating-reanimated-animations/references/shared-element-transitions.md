...
Reanimated 4.2+ exposes Shared Element Transitions behind a static flag. Add it to the app's `package.json`:
...
Then rebuild the native app. For Expo projects that own native builds, run prebuild before rebuilding:

```bash
npx expo prebuild
```

Static flags cannot be changed in Expo Go. Use a development build instead.
