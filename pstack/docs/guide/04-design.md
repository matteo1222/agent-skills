...
[`$pstack arena`](../../skills/arena/SKILL.md) is the general tool underneath. N Codex subagents are spawned from the bundled prompt reference to attempt the same design or code brief, using rolling slots when N exceeds the available fanout. Each writing candidate gets an explicit separate worktree or directory because Codex subagents share one filesystem. A judge spawned with a distinct model or reasoning profile scores every candidate against a rubric under a read-only prompt contract. The coordinator reads each candidate end to end, picks a base, grafts in the best ideas from the losers, and verifies the result.
...
[`$pstack interrogate`](../../skills/interrogate/SKILL.md) sends the same diff, intent, and rubric to reviewers on distinct Codex model and reasoning profiles. The default panel uses `gpt-5.6-sol @ max`, `gpt-5.6-terra @ max`, and `gpt-5.6-luna @ max`; the hardest judging can use `gpt-5.6-sol @ xhigh`. Profile diversity is the point. Different profiles have different blind spots, so a finding two reviewers raise independently is high-confidence signal. The lead sorts everything into `Act on`, `Consider`, `Noted`, and `Dismissed`, with a reason for each dismissal, and applies nothing automatically.
...
`$pstack` already applies this ladder. Boundary-crossing work triggers `$pstack architect` on its own, so you reach for these directly mainly when you want more or less scrutiny than the default.
