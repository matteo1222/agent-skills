...
```text
$pstack this command emits two records after a retry. repro first, then fix and verify.
```

A feature prompt states the behavior and what must not change:

```text
$pstack add a --json flag. text output stays byte-identical. verify both forms.
```

A refactoring prompt pins behavior before structure moves:

```text
$pstack move parsing into one module, zero behavior change. record the current output first and prove it's unchanged after.
```

A perf prompt states the measurement, not a vibe:

```text
$pstack startup takes 1.8s on this fixture. trace it, fix the measured cause, show me before and after.
```
...
## Let `$pstack` load the TypeScript rules

The Codex port keeps leaf skills inert in `$HOME/.agents/pstack`, so they do not auto-register globally. When `$pstack` touches a `.ts` or `.tsx` file, its router loads [`typescript-best-practices`](../../skills/typescript-best-practices/SKILL.md) from the payload and turns the type-system principles into concrete rules: discriminated unions, `unknown` at boundaries, exhaustive variants, schema-derived types.

## Clean before you commit

The [Opening a PR playbook](../../skills/poteto-mode/playbooks/opening-a-pr.md) reviews the diff before each commit for narrating comments, unsupported guards, dead compatibility paths, and unrelated edits, then applies [`$pstack unslop`](../../skills/unslop/SKILL.md) to the PR description and commit bodies. Codex uses its terminal and the pstack principles for the code pass; no companion Cursor plugin is required.
...
You'll develop your own shorthand. The skill reads intent fine from terse prompts like `$pstack unslop that, tighten it`.
...
[`$pstack no-comments`](../../skills/no-comments/SKILL.md) spawns a Codex subagent from the [Comment Sicko](../../agents/comment-sicko.md) prompt reference. Read-only behavior is a prompt contract. The reviewer has a short keep list: license headers, doc comments on a public API, links that explain what code can't, behavior forced by an external dependency you can't reshape. Everything else goes. A surprise in your own code gets no such pass. The comment comes back as a refactor flag, and `$pstack no-comments` fixes the flags it accepts at the root cause. When a comment claims a constraint, "do not remove", the skill offers to encode the claim as a type, test, or lint. Either way, the comment comes out.

The division of labor is worth keeping straight. The pre-commit diff pass cleans slop out of the code, `$pstack unslop` cleans it out of prose, and `$pstack no-comments` hands the comments to a reviewer who didn't write them.

**Pitfall:** cleanup is not optional polish. A diff with narrating comments and defensive dead weight reads as unfinished to reviewers, and the extra code is where the next bug hides. If the diff feels padded, ask `$pstack` to clean the diff before you commit, not after review calls it out.
