...
```text
$pstack im going to bed. migrate every caller to the new parser in a fresh worktree off <base>.
done means zero old callers, all parser fixtures pass, old api deleted.
keep a decision log. don't ask me before committing.
/goal until done. if you're truly stuck after a few hours, stop and write up why.
```

Walk through what each line buys you:

- "im going to bed" asks Codex to keep working within the authority already granted. It does not authorize unrelated writes, merges, messages, or other external changes.
...
- `/goal` is Codex's goal capability, not a pstack skill. The [Autonomous run playbook](../../skills/poteto-mode/playbooks/autonomous-run.md) uses the goal and Codex wait or monitor mechanisms to re-check the finish condition.
- The escape hatch lets it stop at a genuine dead end and write up why, which beats eight hours of creative goal reinterpretation.

Because you'll review this work after stepping away, `$pstack` routes it through [`$pstack figure-it-out`](../../skills/figure-it-out/SKILL.md), which designs the run's phases before any code and wires in the decision log.
...
Before the skill hands back its summary, it spawns a reviewer with a distinct Codex model or reasoning profile, normally `gpt-5.6-sol @ xhigh`, to read the trail and the active task history or supplied digest. The reply ends with an Attention section listing what deserves your scrutiny. Read that section first, then the log rows it points at. You're auditing decisions, not re-reading the whole night.
...
```text
$pstack full autopilot on this queue. each item is independent. i want them merged by morning.
```
...
```text
$pstack autopilot these five changes but stack them, don't ship. i'll land the stack in the morning.
```

[Orchestrate](../../skills/poteto-mode/playbooks/orchestrate.md) is for a program that outlives any single subagent: multi-day, many stacked PRs, fleets of Codex subagents under one standing coordinator task. The coordinator spawns workers from bundled prompt references, authors briefs, collects what its subagents finish, keeps the lowest unmerged PR green, and never writes code itself. All subagents share one filesystem, so every parallel writer gets an explicit worktree. It's deliberately heavy machinery. If one agent could finish the work in a session, the playbook itself routes you back to the overnight contract above:

```text
$pstack orchestrate the store migration. own it until every package is converted and merged. i'll check in twice a day.
```

**Pitfall:** a duration is not a finish condition. "work on this for 4 hours" gives the agent nothing to check, and you'll wake up to four hours of motion instead of a result. Give Codex `/goal` a predicate that can pass or fail.
