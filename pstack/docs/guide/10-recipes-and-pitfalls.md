...
![She tastes a finished dish while robots cook from a recipe box, with pinned cards reading $pstack how, $pstack tdd, and /goal above the counter.](./images/recipes.jpg)
...
```text
$pstack repro the duplicate write first. if there's a cheap test path, $pstack tdd it. then fix and rerun.
```
...
```text
$pstack im going to bed, keep going autonomously until every fixture passes. do not stop. keep a decision log i can audit in the morning. use /goal until done.
```
...
```text
$pstack i said the goal is to repro. i did not ask for a fix yet.
```

```text
$pstack apply prove it works. show me the real output, not the build log.
```
...
- **A vague finish condition.** "make it better" gives `/goal` nothing to check. Give a command or artifact that can pass or fail.
- **Parallel agents in one worktree.** Codex subagents share one filesystem, so writers can overwrite each other and the diff becomes archaeology. Say "own worktree per attempt" and require the coordinator to create that isolation explicitly.
...
- **Treating `auto` as a model slug.** `auto` and `inherit-parent` mean "omit model and reasoning overrides so Codex resolves its configured subagent defaults." The historical alias does not force literal parent inheritance. Otherwise specify both parts, for example `gpt-5.6-luna @ max` or `gpt-5.6-sol @ xhigh`. [Setup](./01-setup.md) covers the roles.
