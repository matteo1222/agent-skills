...
pstack ships 21 principles as payload instructions. `$pstack` reads their index at the start of every multi-step invocation, applies the ones the task triggers, and names each applied principle in its reply along with the decision it changed. The leaf files stay inert under `$HOME/.agents/pstack`; they do not register as separate global Codex skills.

You don't invoke the principle files directly. Reinvoke `$pstack` and use their names to steer. Each name points at a complete rule the agent reads for that turn, so one phrase redirects the work more precisely than a paragraph of instructions.
...
```text
$pstack use subtract before you add. delete the obsolete adapters first, then design what's left.
```

Say it claims success because the build passed:

```text
$pstack apply prove it works. run the real import flow and show me the written records.
```

Say two parallel attempts are about to write to the same branch:

```text
$pstack separate before serializing shared state. give each attempt its own worktree, no locks.
```
...
- [Guard the Context Window](../../skills/principle-guard-the-context-window/SKILL.md) routes bulk reading to subagents and keeps findings in the main task.
