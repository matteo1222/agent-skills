...
Set the completed user-facing feature map path in `control.feature_map_path`. Copy and fill [`feature-map.example.md`](./feature-map.example.md) outside `.codex/pstack/benny/pack/` instead of editing the copied example.
...
Before enabling the repro scheduled task, run one harmless adapter check after the user explicitly requests the test:
