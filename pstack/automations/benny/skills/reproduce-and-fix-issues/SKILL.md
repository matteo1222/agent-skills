...
description: Reproduce triaged Slack bugs through a configured app-control adapter, verify existing fixes, and open a bounded draft pull request only after before-and-after proof. Use only from the configured Benny Codex scheduled task.
...
Load the external Benny configuration supplied by the scheduled-task prompt. Inspect the live Slack, tracker, GitHub, and control tools and follow only their exposed schemas. If the config, an exact required action, a connected app, the control adapter, or the completed feature map is missing, fail closed.

## Hard safety rules

- Run external writes only from a user-created and enabled Benny scheduled task whose prompt explicitly authorizes those writes. In an ordinary setup or inspection task, produce a dry-run report instead.
- Never invent a connector action, tool argument, private endpoint, or backend call. If the live schema cannot perform a required operation, fail closed.
...
Before proceeding, search this exact source thread and the configured operations channel for a prior Benny repro result, verification result, or linked draft pull request for the same source permalink. If a deterministic prior result exists, stop without repeating it. If the scheduled poll cannot determine whether this report was already processed, fail closed instead of risking duplicate work or posts.
...
Prefer the configured live Slack connector actions and follow their exact schemas. Use `BENNY_SLACK_BOT_TOKEN` only when the user explicitly configured and authorized it for a narrow missing capability such as editing this one status message. Never expose the token to a worker.

If no operations channel is configured, keep detailed status in the scheduled-task run output. Do not substitute a source-channel root message.
...
Use `$pstack how` to trace the action through the repository. Use `$pstack why` for regression history and defensive code. Form competing cause hypotheses and identify evidence that would separate them.
...
- Invoke `$pstack tdd` when there is a cheap local test target, and write the failing test before the fix.
...
- Run the pull request text and all Slack updates through `$pstack unslop`.
