...
description: Triage Slack issue reports with one thread-only verdict, evidence review, cause-aware routing, tracker dedupe, and fail-closed ticket creation. Use only from the configured Benny Codex scheduled task.
...
Load the external Benny configuration supplied by the scheduled-task prompt. Inspect the live Slack and tracker tools and follow only their exposed schemas. If the config, an exact required action, or its connected app is missing, malformed, or incomplete, stop without posting or writing to the tracker.
...
Do a bounded source and history pass before choosing an owner or destination. Use `$pstack how` to trace the path from the reported action to the observed result. Use `$pstack why` when the report looks like a regression or touches defensive code.
...
Use only the configured marker strings. The repro scheduled task trusts the marker only when it comes from the configured triage identity in this source thread.
...
If this run created a tracker issue and the verdict did not land, use the adapter's compensation action. Verify that the issue is canceled, closed, or deleted. If compensation cannot be verified, report the failure only in the scheduled-task run output.
...
- Stop early if someone asks the scheduled task to stop.
