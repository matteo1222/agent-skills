...
**pstack is my answer.** these are the same skills i use everyday to ship high quality code at Cursor. this Codex port turns Codex into a rigorous engineering team. the goal is not to maximize loc, in fact it's the opposite. pstack helps you write less, but higher quality code.

**pstack gives you fearless parallelism.** when you can go deep on one agent and trust it to write good, verifiable code, you can truly parallelize with confidence. spawn subagents from the bundled [`poteto-agent`](./agents/poteto-agent.md) prompt reference and trust that they'll apply rigorous engineering principles to their work.

**codex gives you the best of all worlds.** every model and reasoning profile has its strengths and weaknesses. pstack can route each subagent to the profile that fits its role.

fork it. improve it. make it yours. PRs are welcome! 

## install

the Codex port is installed globally as one discoverable shim at `$HOME/.agents/skills/pstack` and an inert payload at `$HOME/.agents/pstack`. keeping the leaf skills in the payload avoids registering forty-eight top-level skills or colliding with skills you already have. invoke everything through `$pstack`.
...
use [`$pstack`](./skills/poteto-mode/SKILL.md) at the start of a task. it reads your request, picks from a set of playbooks, and runs the other skills as the steps need them.

### just use [`$pstack`](./skills/poteto-mode/SKILL.md)
...
```
$pstack this pr has a subtle bug where the scroll drifts every 750ms even when idle. repro
first, then fix and verify.
```

```
$pstack i'm going to bed. land the stack even if ci flakes. i want everything merged by
morning.
```
...
1. opens a plan. the first item is reading the inline principles index in the skill.
...
Codex skills are selected per turn, so `$pstack` is not sticky. invoke it again on each follow-up where you want the mode to apply, for example `$pstack continue`.

[`$pstack`](./skills/poteto-mode/SKILL.md) works extremely well with Codex's `/goal` capability. a goal can keep Codex checking a finish condition over a long run without sacrificing rigor; it does not expand the permissions or authority in your request.

## skills

[`$pstack`](./skills/poteto-mode/SKILL.md) runs most of these for you when a step needs them (`how`, `why`, `architect`, `arena`, `swarm`, `interrogate`, `unslop`, `no-comments`, `technical-writing`, `tdd`, and the principles). the table below is for when you want one directly:
...
| skill | use it when |
|---|---|
| [`$pstack`](./skills/poteto-mode/SKILL.md) | default entry point for any non-trivial task. |
| [`$pstack how`](./skills/how/SKILL.md) | you want a walkthrough of how a subsystem works. |
| [`$pstack why`](./skills/why/SKILL.md) | you want to know why something was built this way. discovers available Codex tools and connectors at run time and queries each available evidence category in parallel (source control, issue tracker, long-form docs, real-time chat, infra observability, error tracking, analytics warehouse). |
| [`$pstack recall`](./skills/recall/SKILL.md) | you're starting or resuming work and want context rebuilt from the active Codex task or thread history, a user-provided digest, and the shared record, handed back as a tight current-state brief. |
...
| [`$pstack make-bot-ui`](./skills/grokbot/make-bot-ui/SKILL.md) | legacy Grok Bot UI workflow retained from upstream; use only when compatible webhook and state-update tooling is available. |
| [`$pstack setup`](./skills/setup-pstack/SKILL.md) | you want to pick which Codex model and reasoning effort pstack uses per role. checks the profiles advertised in the current session and writes an override file. |
...
| [`$pstack maintain-verification-skill`](./skills/maintain-verification-skill/SKILL.md) | your verify skill's feature map has drifted from the app. source wave + one live pass, one reviewable set of proven corrections; opens a PR only when asked. |
...
mostly i type [`$pstack`](./skills/poteto-mode/SKILL.md) at the start of a task and let it route to a playbook. the other skills fire as the steps need them. a few i reach for directly.
...
```
bug fix:           $pstack this pr has a subtle bug where the scroll drifts every 750ms even
                   when idle. repro first, then fix and verify.
perf:              $pstack a big list takes a second or two to load even though we virtualize.
                   run a cpu trace and tell me why.
feature:           $pstack build a small feature behind a feature flag. verify it really works.
prototype:         $pstack build two prototypes of the markdown renderer so we can compare.
                   spawn an agent for each.
multi-phase:       $pstack open source these skills as a plugin. nothing internal leaks, work
                   in a temp dir, show me the dependency graph first.
overnight run:     $pstack i'm going to bed. land the stack even if ci flakes. i want
                   everything merged by morning.
babysit:           $pstack check on pr 123. anything outstanding?
visual parity:     $pstack the row spacing is too tall when this flag is on. the second image
                   is correct. repro and fix until it matches.
figure it out:     $pstack i'm stepping away. migrate every caller from the synchronous store
...
pstack also ships a subagent prompt that runs my style end to end. when spawning a Codex subagent, point its task at [`agents/poteto-agent.md`](./agents/poteto-agent.md) and require it to read that prompt before doing any work. choose the model and reasoning effort as separate spawn settings. the prompt reads `poteto-mode` in full, including its inline principles index.

[`$pstack`](./skills/poteto-mode/SKILL.md) and a subagent spawned from the `poteto-agent` prompt route through the same wrapper.

pstack also ships [Comment Sicko](./agents/comment-sicko.md), a read-only comment-review prompt. usually invoke it through [`$pstack no-comments`](./skills/no-comments/SKILL.md), which spawns the reviewer from that prompt reference.
...
## Codex-native dependencies

the port maps the upstream host integrations to Codex-native capabilities:

- code cleanup is performed from the diff using pstack's principles; `$pstack unslop` remains the prose cleanup skill.
- CLI and TUI work uses the Codex terminal; browser, Electron, and web work uses the available Codex browser tooling.
- skill authoring routes through Codex's `skill-creator` skill.
- subagents are spawned from the bundled prompt references. read-only behavior is a prompt contract, and parallel writers need explicit separate worktrees because all subagents share one filesystem.

## why are there no planning skills?

codex already has a plan capability which works great with pstack. but personally, i don't believe in planning. the best spec is code. if you do want to make a plan, [`$pstack`](./skills/poteto-mode/SKILL.md) covers it, but it's not a default. 
...
pstack also preserves the dormant upstream [benny automation pack](./automations/benny/) for provenance. it is not registered as a Codex skill or automation. use it only after adapting its Slack, webhook, browser, and repository integrations to tools you have explicitly connected, and create a Codex automation only when the user explicitly asks for one.
