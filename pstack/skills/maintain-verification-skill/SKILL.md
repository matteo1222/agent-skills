...
description: "Periodic pass that keeps a project's verification skill and feature map honest: parallel source readers per feature, one live session driving every feature, and one reviewable set of proven corrections. Use for $pstack maintain-verification-skill or \"audit the verify skill\"."
...
- **changed** — one reviewable change set contains proven doc, harness, or map corrections; publish it as a PR only when the user authorized that external action.
...
2. **Source wave.** Spawn one subagent per feature file concurrently, subject to available slots. Give each `spawn_agent` call a standalone brief with `fork_turns: "none"`, an available model/effort pair, and `Do not edit files, drive the app, or mutate external state`. Each explains "how does this user-facing feature work?" from source, flags likely doc drift with citations, and returns one concise live-verification recipe in its result. Return shape: feature summary / source entry points / likely drift or none / one recipe.
...
6. **Ship or stop.** For changed: re-read every changed file first and present one local change set. Commit, push, or open one PR only when the user explicitly placed those actions in scope. For clean or blocked: no PR, report the outcome and the coverage honestly.
