...
Create a Codex task plan with one entry per phase before starting. Autonomous mode without checkpoints needs the plan to show phase position and keep phases from silently disappearing.
...
Use your configured architect runners (defaults `gpt-5.6-sol @ max`, `gpt-5.6-terra @ max`, `gpt-5.6-luna @ max`).
