...
2. **Identify consensus**. Findings raised independently by 2+ configured arms are highest signal.
3. **Identify single-arm findings**. Still worth reading, but weight accordingly.
4. **Deduplicate**. Different arms may describe the same issue differently. Merge these and note which arms raised it.
5. **Note disagreements**. If one arm flags something and another explicitly says the opposite, that's useful context for the verdict.
