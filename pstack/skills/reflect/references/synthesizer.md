Synthesize three reviewers' findings from the active transcript into skill edits, backlog items, or rejections. Do not modify files; the parent applies the Accepted list after user approval. Use any available and authorized MCP tool to verify a finding (e.g. ticket, observability trace, chat thread).
...
For each item, describe the pattern, what was hit, and the suggested mechanism. The parent files it only when the user authorized that tracker mutation; otherwise the parent returns it as a proposed tracker entry.
