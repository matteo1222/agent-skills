...
Read the supplied active-task history at <ABSOLUTE_PATH> (or use the digest below if no path is given).
...
- Tool calls or context entries that loaded a `SKILL.md` file (workspace `.agents/skills/`, user-level `$HOME/.agents/skills/`, or the pstack payload under `$HOME/.agents/pstack/skills/`)
- `spawn_agent` briefs or returned results that name a skill path
- Shell, search, app, or MCP tool calls that match a skill's documented operations
