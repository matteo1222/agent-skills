...
- Every spawn and every follow-up carries the standing orders verbatim.
- The brief is the product. A vague brief fails quietly, because a worker cannot ask you a question.

Create a Codex task plan with `update_plan` and copy the steps below into it verbatim. A step you skip stays listed with `skip: <reason>`.

#### Roles and placement

- **Coordinator (this task).** Frames, authors briefs, drains the inbox, owns the human report, and makes judgment calls. It never authors or edits code: conflicted merges, restacks, and code changes are always agent tasks. Mechanically landing a verified unit (fast-forward or clean cherry-pick of a worker's commit, then push) is bookkeeping the coordinator may do itself on repos where local git is cheap; queueing finished work behind an idle stacker is how a deadline harvests nothing. The loop is agentic end to end. Spawn with `spawn_agent`, inspect state with `list_agents`, wait for mailbox changes with bounded `wait_agent` calls, and use `followup_task` only to assign new work or clarify an idle agent. State reads and writes go through `skills/poteto-mode/scripts/orch/orch.ts` at drain points, one command in and one line out, to conserve context. Resolve that script to an absolute path from the loaded pstack payload and keep the command's working directory at the user's repository. The CLI never spawns, waits, or wakes anything.
...
Create `orchestrate/<project-slug>/` in the durable state directory authorized for the task, and record its absolute path. Every file has exactly one writer; owners publish facts, readers aggregate at read time. Use `bun "<absolute-pstack-payload>/skills/poteto-mode/scripts/orch/orch.ts"` for bookkeeping, written below as `orch`, while the command stays rooted in the user's repository and its canonical plain TSV and JSON stay readable without the CLI.

- `preferences.md` is the standing-orders register: numbered lines, one constraint each (model policy, stack shape and count, verification bar, forbidden paths, escalation policy). Paste it verbatim into every spawn and every follow-up; directives decay across turns, and each dropped one costs a human turn. When you catch yourself restating an instruction, append the line before you act (principle-encode-lessons-in-structure).
...
CONTEXT      pointers to files and PRs; upstream reports pasted in full when this unit
             depends on them, because shared files do not provide sibling reasoning or reports
ACCEPTANCE   checkable criteria, one per line
VERIFY       exact commands or an available Codex control path, plus known gotchas
...
Size the brief to the unit. A one-command unit gets the template collapsed to a paragraph that still names goal, scope, the verify command, and the report shape; a 4KB scaffold around a two-line edit costs more to write and obey than the edit. Spawns may reference the standing-orders file by its absolute store path, but every follow-up repeats the standing orders verbatim.

A sub-coordinator brief adds its track boundary and unit list, its spawn budget and worktree allocation, the drain protocol, and the rollup format (per child: name, status, PR, head SHA, verdict, one line; plus track status and frontier delta).

A dependency is a context relay, not just ordering: undeclared upstream context makes the worker guess. Missing fields are a refuse-to-spawn condition. Audit one sampled worker brief per sub-coordinator per wave, concurrently with the wave it samples, never as a gate in front of it; a failing brief stops that track and fixes the sub-coordinator's instructions, not just the worker, because brief quality decays late in a run. Never follow-up-chain a changing brief; spawn a fresh agent with consolidated scope.
...
2. **Install the runtime.** Resolve the loaded pstack payload to an absolute path, keep the working directory at the user's repository, and run `orch init`. Open the trail via the show-me-your-work skill, write the standing orders before any spawn, and seed `frontier.json` from existing PRs with `orch frontier set --repo <repo-dir>`.
...
- On a completion notification from the agent mailbox, run `orch inbox push <agent> <unit> <status> [--report PATH]` and return to what you were doing. Never deep-review inline; a completion that needs review becomes a verifier unit. Never review a diff inside a drain.
- Drain in batches at four points: the end of a critical section, a track rollup, a frontier watcher wake (arm it through an explicitly requested Codex goal, monitor, or automation, with a long bounded-wait fallback), and before a human report. Begin each batch with `orch inbox drain`. Arrivals during a drain wait for the next one.
...
- The frontier is a computed object, never narrative. Recompute `frontier.json` from `gt` after every merge and stack mutation because GitHub base refs drift mid-restack while gt tracking is authoritative: ordered PR list, branch names, head SHAs, a generation number, the lowest unmerged PR. Resolve it in the stacker's explicit worktree where gt knows the stack; a checkout whose gt metadata never saw the submits reports no PRs and the command errors rather than guessing.
- Exactly one stacker per stack may run `gt`, serialized within its stack; record the holder and its worktree in the standing orders. Restacks run only in that dedicated worktree; other agents do not touch it.
...
- Never use `followup_task` merely to check on an agent; it starts another turn for an idle agent. Probe read-only with `list_agents`, bounded `wait_agent`, the ledger, `units.tsv`, `gh`, and pushed branches. Transcript mtime is not liveness. Read task or thread history only through an authorized task tool, or use the parent-provided digest.
...
- After a Codex restart: treat prior subagents as unavailable unless the current agent tools show them live. Re-read the standing orders and `units.tsv`, recompute the frontier, reconcile durable work by PR, branch, and head SHA rather than agent id, spawn one fresh sub-coordinator per active track from its stored brief plus current state, then drain and continue. The dead session's store lock clears itself on the next write; `orch` replaces a lock whose holder pid is gone.
