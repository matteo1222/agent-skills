...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent using your configured perf-issue profile (default model `gpt-5.6-sol`, reasoning effort `max`); review the diff. Capture a post-fix trace.
