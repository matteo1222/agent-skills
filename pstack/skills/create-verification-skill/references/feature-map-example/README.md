...
- Create a unique data directory with `NOTES_DATA_DIR="$(mktemp -d)"` and export it so concurrent runs do not share state; remove only that directory during cleanup.
