...
Create a Codex task plan whose first item is to read the Principles section of the **poteto-mode** skill. Then add the phases below as plan items.
...
- For one-way-door design decisions, run the **architect** skill (it runs **arena**) with diverse, isolated, opinionated candidates and a judge on a different configured Codex model/effort variant. The judge's standalone `spawn_agent` brief must say `Do not edit files or mutate external state`. Skip it for mechanical work whose shape is already concrete. A second arena over a settled design is over-engineering (the **laziness-protocol** principle skill).
...
Then execute the design. Add its steps to the Codex task plan as concrete items, after the Phase C entry and before Phase D. Run each under the Phase C loop discipline, and weave the Phase D log through them, a row as each step lands, rather than saving the whole trail for the end.
