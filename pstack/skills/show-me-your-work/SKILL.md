...
Use the helper so rows stay well-formed. Resolve `scripts/log.sh` relative to this `SKILL.md` as an absolute path, keep the shell working directory at the repository or task work directory, then run `<ABS_SKILL_DIR>/scripts/log.sh <logfile> <phase> <decision> <why> <evidence> <result>`. It stamps `ts`, writes the header on first use, strips stray tabs/newlines, and prefixes any cell starting with `=`, `+`, `-`, or `@` with a single quote so a reviewer opening the log in a spreadsheet doesn't trigger formula execution. A bare `printf` appending a row works too, but mind those same bytes if cells come from generated or user-supplied text.
...
At the end of the run, before handing back, check the log told the truth. Use the active Codex task context plus authorized task/thread history when available. Do not glob hidden Codex session storage or unrelated tasks. If full history is unavailable, make a tight digest of the current task and state that limitation. Walk the log against what actually happened:
...
Before handing back, spawn a Codex subagent on a different configured model/effort variant from the primary worker when one is available. Give `spawn_agent` a standalone brief with `fork_turns: "none"`, the chosen model and effort as separate fields, and `Do not edit files or mutate external state`. Self-review is not a substitute; the point is fresh eyes you cannot bring yourself. The subagent reads the audit trail and the authorized task history or digest, then returns flags in its result for what the user should pay attention to. Not a redo of the work, a scan for what's suboptimal or risky.

- Decisions logged with weak or absent evidence.
- Verification steps skipped or claimed without proof in the task record or digest.
